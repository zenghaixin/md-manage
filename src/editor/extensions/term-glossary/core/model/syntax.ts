...
const REMARK_BRACE_LINE_RE =
  /^\s*\{[\t ]*remark[\t ]*:[\t ]*([A-Za-z0-9_-]+)[\t ]*\}\s*$/i
/** 行首或独立出现的 `{remark:id}`（允许同行还有其它字时只剥掉该 token） */
const REMARK_BRACE_TOKEN_RE =
  /(?:^|[\s\u00a0])\{[\t ]*remark[\t ]*:[\t ]*([A-Za-z0-9_-]+)[\t ]*\}(?=[\s\u00a0]|$)/gi

/**
 * 描述正文里误入的 `{remark:id}`（应在开场行 attrs，不应进 body）。
 * 剥离后返回干净描述与解析到的 id。
 */
export function peelRemarkBraceFromDescription(description: string): {
  description: string
  remarkId: string
} {
  const text = String(description ?? '').replace(/\u00a0/g, ' ')
  if (!text.trim()) return { description: '', remarkId: '' }
  const lines = text.split('\n')
  let remarkId = ''
  const kept: string[] = []
  for (const line of lines) {
    const whole = REMARK_BRACE_LINE_RE.exec(line)
    if (whole) {
      if (!remarkId) remarkId = whole[1]
      continue
    }
    let next = line
    REMARK_BRACE_TOKEN_RE.lastIndex = 0
    next = next.replace(REMARK_BRACE_TOKEN_RE, (full, id: string) => {
      if (!remarkId) remarkId = id
      // 保留前导空白，去掉 token 本身
      const lead = /^\s/.test(full) ? full[0] : ''
      return lead
    })
    next = next.replace(/[ \t]+\n/g, '\n').replace(/[ \t]{2,}/g, ' ').trimEnd()
    if (!next.trim() && !kept.length) continue
    kept.push(next)
  }
  return {
 ...
 * @param preferRemarkId 优先使用的整块备注 id（避免仅存在于 body 时保存丢失）
 * 找不到则原样返回。
 */
export function replaceTermBlock(
  ...
  preferRemarkId?: string,
): string {
  const target = String(oldTitle ?? '').trim()
  if (!target) return markdown
  const peeled = peelRemarkBraceFromDescription(newDescription)
  ...
  const next = markdown.replace(
    ...
    (full, title: string, braceInner: string) => {
      ...
      found = true
      const remarkId =
        String(preferRemarkId || '').trim() ||
        peeled.remarkId ||
        parseTermRemarkIdFromBrace(braceInner || '')
      return formatTermSource(newTitle, peeled.description, remarkId)
    },
  )
