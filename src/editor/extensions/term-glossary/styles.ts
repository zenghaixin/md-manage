...
.ext-term-conflict-drawer-panel {
  pointer-events: auto;
  position: absolute;
  top: 0;
  right: 0;
  ...
  background: var(--surface, #f4f7f9);
  color: var(--ink, #1a2830);
  border-left: 1px solid var(--border, #c5d0d8);
  box-shadow: -12px 0 32px rgba(26, 40, 48, 0.18);
}

.ext-term-conflict-drawer-header {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  box-sizing: border-box;
  /* 与 AppLayout header（h-12）底边对齐 */
  height: 3rem;
  padding: 0 0.85rem;
  border-bottom: 1px solid var(--border, #c5d0d8);
}

.ext-term-conflict-drawer-title {
  flex: 1;
  font-weight: 700;
  font-size: 0.95rem;
}

.ext-term-conflict-drawer-close {
  border: none;
  background: transparent;
  font-size: 1.25rem;
  cursor: pointer;
...
.ext-term-conflict-drawer-hint {
  padding: 0.5rem 0.85rem;
  font-size: 0.8rem;
  color: var(--muted, #5a6b75);
}

.ext-term-conflict-drawer-body {
  flex: 1;
  overflow: auto;
  padding: 0.35rem 0.65rem 0.85rem;
}

.ext-term-conflict-drawer-footer {
  display: flex;
  flex-wrap: wrap;
  gap: 0.4rem;
...
.ext-term-conflict-row {
  ...
  border-bottom: 1px solid color-mix(in srgb, var(--border, #c5d0d8) 70%, transparent);
}

.ext-term-conflict-check {
  grid-column: 1;
  grid-row: 1;
  align-self: center;
  margin: 0;
...
.ext-term-conflict-pick {
  grid-column: 3;
  grid-row: 1;
  align-self: center;
}

.ext-term-conflict-context mark {
...
.ext-term-conflict-path:hover,
.ext-term-conflict-pick:hover {
  color: color-mix(in srgb, var(--accent, #0d6e6e) 70%, var(--ink, #1a2830));
}

.ext-term-conflict-btn {
  border: 1px solid var(--border, #c5d0d8);
  border-radius: 6px;
  background: #2563eb;
  color: #fff;
...
.ext-term-conflict-btn.is-muted {
  background: transparent;
  color: inherit;
}

.ext-term-conflict-chars {
  display: flex;
  flex-wrap: wrap;
  gap: 0.2rem;
  margin: 0.5rem 0;
...
.ext-term-conflict-char.is-selected {
  background: color-mix(in srgb, #2563eb 18%, transparent);
  border-color: #2563eb;
  color: #2563eb;
}

.ext-term-conflict-pick-actions {
  display: flex;
  gap: 0.4rem;
}
`
